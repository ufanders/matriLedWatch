...
ANDERS NELSON	2010.05.28b ADDED TRISTATE TO LED SINKS TO DISCHARGE
							LED CAPACITANCE WHICH CAUSES GHOSTING,
							ADDED FASTCLOCK()
***********************************************************************/
#include <pic.h>
...
{	
	...
	for(characterScanColumn = characterScanColumnOffset; characterScanColumn < (5 - characterScanColumnOffset); characterScanColumn++) //for use with 5x7 font
	{
		...
		for(characterScanRow = characterScanRowOffset; characterScanRow < (7 - characterScanRowOffset); characterScanRow++) //for use with 5x7 font
		{
			...
			{
				bitmapPutPixel(characterScanColumn, characterScanRow, r, g, b);
			}
...
void showTime(void)
{
	...
	for(currentAnimationFrame = 0; currentAnimationFrame < 10; currentAnimationFrame++)
	{
		if(currentAnimationFrame < 8) //covering entire display for indicators 0-8
		{
			...
			if(elementHourTens != 0)
			{
				...
				{
					bitmapPutPixel(0, currentAnimationFrame, 0, 0, MATRIX_PWM_MAX);
					bitmapPutPixel(1, currentAnimationFrame, 0, 0, MATRIX_PWM_MAX);
				}
			}
			...
			if(elementHourUnits != 0)
			{
				...
				{
					bitmapPutPixel(2, currentAnimationFrame, 0, 0, MATRIX_PWM_MAX);
					bitmapPutPixel(3, currentAnimationFrame, 0, 0, MATRIX_PWM_MAX);
				}
			}
			...
			if(elementMinuteTens != 0)
			{
				...
				{
					bitmapPutPixel(4, currentAnimationFrame, 0, 0, MATRIX_PWM_MAX);
					bitmapPutPixel(5, currentAnimationFrame, 0, 0, MATRIX_PWM_MAX);
				}
			}
			...
			if(elementMinuteUnits != 0)
			{
				...
				{
					bitmapPutPixel(6, currentAnimationFrame, 0, 0, MATRIX_PWM_MAX);
					bitmapPutPixel(7, currentAnimationFrame, 0, 0, MATRIX_PWM_MAX);
				}
			}
		}
		else //backing up into display for indicator 9
		{
			...
			if(elementHourTens != 0)
			{
				if(elementHourTens > currentAnimationFrame)
				{
					bitmapPutPixel(0, 7, MATRIX_PWM_MAX, 0, 0);
					bitmapPutPixel(1, 7, MATRIX_PWM_MAX, 0, 0);
				}
			}	
			...
			if(elementHourUnits != 0)
			{
				if(elementHourUnits > currentAnimationFrame)
				{
					bitmapPutPixel(2, 7, MATRIX_PWM_MAX, 0, 0);
					bitmapPutPixel(3, 7, MATRIX_PWM_MAX, 0, 0);
				}
			}
			...
			if(elementMinuteTens != 0)
			{
				if(elementMinuteTens > currentAnimationFrame)
				{
					bitmapPutPixel(4, 7, MATRIX_PWM_MAX, 0, 0);
					bitmapPutPixel(5, 7, MATRIX_PWM_MAX, 0, 0);
				}
			}
			...
			if(elementMinuteUnits != 0)
			{
				if(elementMinuteUnits > currentAnimationFrame)
				{
					bitmapPutPixel(6, 7, MATRIX_PWM_MAX, 0, 0);
					bitmapPutPixel(7, 7, MATRIX_PWM_MAX, 0, 0);
				}
			}
		}
	...
	{	
		...
		{	
			...
			{
				bitmapPutPixel(xLast, yLast, tempPixelR, tempPixelG, tempPixelB); //restore the last pixel
		...
		{
			bitmapPutPixel(x, y, currentAnimationFrameSigned, currentAnimationFrameSigned, 0);
			timerTicks = 0;
	...
	for(currentAnimationFrame = 1; currentAnimationFrame < (MATRIX_ROWS+1); currentAnimationFrame++)
	{
		...
		{
			bitmapPutPixel(x, (MATRIX_ROWS - currentAnimationFrame), 0, 0, 0); 
		}
